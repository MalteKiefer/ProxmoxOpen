...
import de.kiefer_networks.proxmoxopen.data.api.mapper.buildCluster
...
class ClusterRepositoryImpl @Inject constructor(
    ...
    private suspend fun <T> execute(
        serverId: Long,
        block: suspend (de.kiefer_networks.proxmoxopen.data.api.ProxmoxApiClient) -> T,
        ...
        val credentials = when (server.realm) {
            de.kiefer_networks.proxmoxopen.domain.model.Realm.PVE_TOKEN -> {
                val secret = servers.getTokenSecret(serverId)
                    ?: return ApiResult.Failure(ApiError.Auth("token missing"))
                de.kiefer_networks.proxmoxopen.domain.model.Credentials.ApiToken(
...
} catch (e: de.kiefer_networks.proxmoxopen.data.api.ProxmoxHttpException) {
    when (e.code) {
...
} catch (e: de.kiefer_networks.proxmoxopen.data.api.tls.FingerprintMismatchException) {
