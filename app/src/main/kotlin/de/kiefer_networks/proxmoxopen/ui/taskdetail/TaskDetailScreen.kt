package de.kiefer_networks.proxmoxopen.ui.taskdetail
...
fun TaskDetailScreen(
    ...
    Scaffold(
        ...
        Column(Modifier.padding(padding).fillMaxSize()) {
            if (state.isPolling) LinearProgressIndicator(modifier = Modifier.fillMaxWidth().height(2.dp), color = MaterialTheme.colorScheme.primary)
            when {
                state.isLoading -> LoadingState()
                state.error != null && state.task == null -> ErrorState(state.error?.message ?: "", stringResource(R.string.retry), viewModel::refresh)
                else -> {
                    val scrollState = rememberScrollState()
                    val logLineCount = state.logLines.size

                    // Auto-scroll to bottom when new log lines arrive during polling
                    LaunchedEffect(logLineCount) {
                        if (state.isPolling) {
                            ...
                            Card(Modifier.fillMaxWidth(), shape = RoundedCornerShape(16.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerHigh)) {
                                Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                    Row(verticalAlignment = Alignment.CenterVertically) {
                                        ...
                                        if (state.isPolling) {
                                            StatusBadge(stringResource(R.string.task_live), BadgeTone.Running)
